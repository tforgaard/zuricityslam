...
def main(descriptors, output, num_matched,
    ...
    N, M = len(query_names), len(db_names)
    self = np.zeros((N, M), dtype=bool)
    for k in range(-window_size, window_size):
        self += np.eye(N, M, k=k, dtype=bool)
    
    # Avoid matching with pairs already found from sequential pairs
